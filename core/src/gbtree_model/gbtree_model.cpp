...
namespace quickscorer {
    ...
    unique_ptr<GBTreeModel> GBTreeModel::from_xgb_bin(const string &model_path, int num_trees) {
        xgboost::Predictor xgb_predictor;
        xgb_predictor.Load(model_path);

        auto model = unique_ptr<GBTreeModel>(new GBTreeModel());
        const auto &xgb_trees = xgb_predictor.gbm_->trees;
        uint64_t feature_size = 0;
        int tree_count = 0;
        for (int i = 0; i < xgb_trees.size(); ++i) {
            unique_ptr<InnerNode> root = unique_ptr<InnerNode>(new InnerNode(0));
            auto tree_features = unique_ptr<google::dense_hash_set<uint64_t>>(new google::dense_hash_set<uint64_t>());
            tree_features->set_empty_key(std::numeric_limits<uint64_t>::max());
            GBTreeModel::expand_children(xgb_trees[i]->nodes, xgb_trees[i]->nodes[0], root, *tree_features, feature_size);
            model->_trees.push_back(std::move(root));
    ...
    void GBTreeModel::expand_children(const vector<xgboost::RegTree::Node> &xgb_nodes,
                                      const xgboost::RegTree::Node &assigner, unique_ptr<InnerNode> &assignee,
                                      google::dense_hash_set<uint64_t> &tree_features, uint64_t &feature_size) {
        auto left_child = xgb_nodes[assigner.cleft()];
        auto right_child = xgb_nodes[assigner.cright()];

        assignee->_default_to_left = assigner.default_left();
        assignee->_split_feature = assigner.split_index();
        assignee->_split_value = assigner.split_cond();
        tree_features.insert(assignee->_split_feature);
        if (feature_size < assignee->_split_feature) feature_size = assignee->_split_feature;

        if (left_child.is_leaf()) {
            assignee->_left_child = unique_ptr<LeafNode>(new LeafNode(assignee->_depth + 1, left_child.leaf_value()));
        } else {
            auto left_inner_node = unique_ptr<InnerNode>(new InnerNode(assignee->_depth + 1));
            expand_children(xgb_nodes, left_child, left_inner_node, tree_features, feature_size);
            assignee->_left_child = std::move(left_inner_node);
        }

        if (right_child.is_leaf()) {
            assignee->_right_child = unique_ptr<LeafNode>(new LeafNode(assignee->_depth + 1, right_child.leaf_value()));
        } else {
            auto right_inner_node = unique_ptr<InnerNode>(new InnerNode(assignee->_depth + 1));
            expand_children(xgb_nodes, right_child, right_inner_node, tree_features, feature_size);
            assignee->_right_child = std::move(right_inner_node);
        }
    }
