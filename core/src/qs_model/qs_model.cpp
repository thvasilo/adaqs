...
namespace quickscorer {
    ...
    unique_ptr<QSModel> QSModel::from_xgb_txt(string model_path, uint32_t block_size, bool adaptive, int num_trees) {
        ...
        return QSModel::from_gbdt_model(gbtree_model, block_size, adaptive);
    }

    unique_ptr<QSModel> QSModel::from_xgb_bin(string model_path, uint32_t block_size, bool adaptive, int num_trees) {
        auto gbtree_model = GBTreeModel::from_xgb_bin(model_path, num_trees);
        return QSModel::from_gbdt_model(gbtree_model, block_size, adaptive);
    }

    unique_ptr<QSModel> QSModel::from_gbdt_model(unique_ptr<GBTreeModel> &gbtree_model, uint32_t block_size, bool adaptive) {
        auto logger = spdlog::get(QSLog::_logger_name);
        logger->info("Create quickscorer model with block size {} and adaptive {}.", block_size, adaptive);
        if (gbtree_model != nullptr) {
            try {
                ...
                logger->error("Fail to load QS model from gbtree model, detail: {}.", ex.what());
            }
        }
        return nullptr;
    }
