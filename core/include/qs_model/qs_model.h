...
namespace quickscorer {
    ...
    public:
        ...
        static unique_ptr<QSModel>
        from_xgb_bin(string model_path, uint32_t block_size, bool adaptive = true, int num_trees = 0);

        static unique_ptr<QSModel>
        from_gbdt_model(unique_ptr<GBTreeModel> &gbtree_model, uint32_t block_size, bool adaptive);
    };
