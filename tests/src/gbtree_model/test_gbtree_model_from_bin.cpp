...
namespace quickscorer {
    ...
    class TestGBTreeModelFromBin : public ::testing::Test {
    protected:
    ...
    TEST_F(TestGBTreeModelFromBin, from_xgb_bin) {
        auto model = GBTreeModel::from_xgb_bin("tests/data/agaricus/model.bin");
        ...
        // check the first tree
        {
            ...
            EXPECT_NEAR(node_0->_split_value, 0, 1E-5);
            ...
            EXPECT_NEAR(node_1->_split_value, 0, 1E-5);
        ...
        // check the second tree
        {
            ...
            EXPECT_NEAR(node_3->_value, 1.1457, 1E-5);
            EXPECT_EQ(node_3->_depth, 2);
            ...
            EXPECT_NEAR(node_7->_value, -6.87558, 1E-5);
            EXPECT_EQ(node_7->_depth, 3);

            auto node_8 = dynamic_cast<LeafNode *>(node_4->_right_child.get());
            EXPECT_NEAR(node_8->_value, -0.127376, 1E-5);
            EXPECT_EQ(node_8->_depth, 3);
            ...
            EXPECT_NEAR(node_9->_value, -0.0386054, 1E-5);
            EXPECT_EQ(node_9->_depth, 3);

            auto node_10 = dynamic_cast<LeafNode *>(node_5->_right_child.get());
            EXPECT_NEAR(node_10->_value, -1.15275, 1E-5);
            EXPECT_EQ(node_10->_depth, 3);

            auto node_6 = dynamic_cast<LeafNode *>(node_2->_right_child.get());
            EXPECT_NEAR(node_6->_value, 0.994744, 1E-5);
            EXPECT_EQ(node_6->_depth, 2);
        }
    ...
    TEST_F(TestGBTreeModelFromBin, num_trees) {
        auto model1 = GBTreeModel::from_xgb_bin("tests/data/agaricus/model.bin", 1);
        EXPECT_EQ(model1->_trees.size(), 1);
        auto model2 = GBTreeModel::from_xgb_bin("tests/data/agaricus/model.bin", 2);
        EXPECT_EQ(model2->_trees.size(), 2);
        auto model3 = GBTreeModel::from_xgb_bin("tests/data/agaricus/model.bin", 0);
        EXPECT_EQ(model3->_trees.size(), 2);
    }

    TEST_F(TestGBTreeModelFromBin, predict) {
        auto model = GBTreeModel::from_xgb_bin("tests/data/agaricus/model.bin");
        ...
        while (!_samples.eof() && !_preds.eof()) {
            ...
            // check consistency
            EXPECT_NEAR(pre_predicted, predicted, 1E-5);
        }
    }
}